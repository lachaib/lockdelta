...
describe('generateMarkdown', () => {
  ...
  it('formats added packages with version', () => {
    const changes: PackageChange[] = [
      { name: 'httpx', change_type: 'added', old_version: null, new_version: '0.27.0', is_direct: false, is_dev: false },
    ];
  ...
  it('formats updated packages with old → new versions', () => {
    const changes: PackageChange[] = [
      { name: 'requests', change_type: 'updated', old_version: '2.31.0', new_version: '2.32.3', is_direct: false, is_dev: false },
    ];
    const md = generateMarkdown(makeReport(changes));
  ...
  it('formats removed packages with old version', () => {
    const changes: PackageChange[] = [
      { name: 'urllib3', change_type: 'removed', old_version: '1.26.15', new_version: null, is_direct: false, is_dev: false },
    ];
    const md = generateMarkdown(makeReport(changes));
  ...
  it('bolds direct prod dependencies', () => {
    const changes: PackageChange[] = [
      { name: 'requests', change_type: 'updated', old_version: '2.31.0', new_version: '2.32.3', is_direct: true, is_dev: false },
    ];
    const md = generateMarkdown(makeReport(changes));
  ...
  it('italicizes dev dependencies', () => {
    const changes: PackageChange[] = [
      { name: 'pytest', change_type: 'updated', old_version: '8.0.0', new_version: '8.1.0', is_direct: true, is_dev: true },
    ];
    const md = generateMarkdown(makeReport(changes));
  ...
  it('leaves transitive deps as plain links', () => {
    const changes: PackageChange[] = [
      { name: 'urllib3', change_type: 'updated', old_version: '2.2.1', new_version: '2.2.2', is_direct: false, is_dev: false },
    ];
    const md = generateMarkdown(makeReport(changes));
  ...
  it('links javascript packages to npmjs', () => {
    const changes: PackageChange[] = [
      { name: 'express', change_type: 'updated', old_version: '4.18.2', new_version: '4.19.2', is_direct: false, is_dev: false },
    ];
  ...
  it('links scoped npm packages correctly', () => {
    const changes: PackageChange[] = [
      { name: '@babel/core', change_type: 'updated', old_version: '7.23.9', new_version: '7.24.5', is_direct: false, is_dev: false },
    ];
    const md = generateMarkdown(makeReport(changes, 'javascript'));
  ...
  it('links deno jsr packages to jsr.io', () => {
    const changes: PackageChange[] = [
      { name: 'jsr:@std/path', change_type: 'updated', old_version: '0.224.0', new_version: '0.225.2', is_direct: false, is_dev: false },
    ];
  ...
  it('includes all three sections when all change types are present', () => {
    const changes: PackageChange[] = [
      { name: 'added-pkg', change_type: 'added', old_version: null, new_version: '1.0.0', is_direct: false, is_dev: false },
      { name: 'updated-pkg', change_type: 'updated', old_version: '1.0.0', new_version: '2.0.0', is_direct: false, is_dev: false },
      { name: 'removed-pkg', change_type: 'removed', old_version: '1.0.0', new_version: null, is_direct: false, is_dev: false },
    ];
    const md = generateMarkdown(makeReport(changes));
    expect(md).toContain('### Added');
    expect(md).toContain('### Changed');
  ...
  it('flattens changes from multiple lockfiles with per-ecosystem links', () => {
    const report: DiffReport = {
      ...
      base_ref: 'main',
      head_ref: 'feature',
      summary: { added: 2, removed: 0, updated: 0, total_changes: 2, ecosystems: ['python', 'javascript'] },
      lockfiles: [
        {
          path: 'uv.lock', workspace: '.', type: 'uv', ecosystem: 'python',
          summary: { added: 1, removed: 0, updated: 0, total_changes: 1 },
          changes: [{ name: 'httpx', change_type: 'added', old_version: null, new_version: '0.27.0', is_direct: false, is_dev: false }],
          migration: null,
        },
        {
          path: 'package-lock.json', workspace: '.', type: 'npm', ecosystem: 'javascript',
          summary: { added: 1, removed: 0, updated: 0, total_changes: 1 },
          changes: [{ name: 'axios', change_type: 'added', old_version: null, new_version: '1.7.0', is_direct: false, is_dev: false }],
          migration: null,
        },
      ],
    };
